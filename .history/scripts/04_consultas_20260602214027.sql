...
SELECT MIN(precoDeFabricacao) AS menorPreco
FROM Produtos;

--Apresente a quantidade de produtos cadastrados em cada categoria.
SELECT SUM(idProduto) AS quantidadeProdutos, nomeCategoria AS categoria
FROM Produtos
JOIN CategoriaProduto ON Produtos.idCategoria = CategoriaProduto.idCategoria
GROUP BY nomeCategoria;

--Exiba a quantidade de ordens de produção cadastradas por funcionário responsável.
SELECT nomeFuncionario AS nome, SUM(idOrdem) AS quantidadeOrdens
FROM Funcionarios
JOIN OrdemProducao ON Funcionarios.idOrdem = OrdemProducao.idOrdem;
WHERE Funcionarios.idOrdem = OrdemProducao.idOrdem;

--Liste o nome dos funcionários e o nome do setor ao qual cada funcionário pertence.
SELECT nomeFuncionario AS nome, nomeSetor AS setor
FROM Funcionarios
...
SELECT nomeProduto AS nome, nomeCategoria
FROM Produtos
JOIN CategoriaProduto ON Produtos.idCategoria = CategoriaProduto.idCategoria;

--Liste o nome do produto, o nome do fornecedor principal e o preço de fabricação do produto.
SELECT nomeProduto AS nome, Fornecedores.idFornecedor AS id, precoDeFabricacao AS preco
FROM Produtos
...
FROM Produtos
WHERE precoDeFabricacao = (
    SELECT MAX(precoDeFabricacao)
    FROM Produtos
);
...
FROM Funcionarios
WHERE salarioFuncionario > (
    SELECT AVG(salarioFuncionario)
    FROM Produtos
); 